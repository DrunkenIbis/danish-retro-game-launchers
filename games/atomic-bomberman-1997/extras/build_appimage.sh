#!/usr/bin/env bash
set -Eeuo pipefail

# Build an AppDir/AppImage that bundles Atomic Bomberman + Wine.
# Build/distribute only if you have rights to the concrete game copy.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
GAME_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
REPO_ROOT="$(cd "$GAME_DIR/../.." && pwd)"

PROJECT_NAME="atomic-bomberman-1997"
DISPLAY_NAME="Atomic Bomberman (1997)"
ARCH="${ARCH:-x86_64}"
APPDIR="${APPDIR:-$SCRIPT_DIR/build/${PROJECT_NAME}.AppDir}"
DIST_DIR="${DIST_DIR:-$SCRIPT_DIR/dist}"
CACHE_DIR="${CACHE_DIR:-$SCRIPT_DIR/.cache-appimage}"
OUTPUT_APPIMAGE="${OUTPUT_APPIMAGE:-$DIST_DIR/${PROJECT_NAME}-${ARCH}.AppImage}"
STATE_DIR_BASENAME="$PROJECT_NAME"
PREFIX_SEED_REL="game/wineprefix"
INTERNAL_LAUNCHER_REL="game/appimage-launch.sh"

SOURCE_BASE="${RETRO_GAME_SOURCE_DIR:-$REPO_ROOT/local/sources}"
RUNTIME_BASE="${RETRO_GAME_RUNTIME_DIR:-$REPO_ROOT/local/runtime}"
RUNTIME_DIR="${AB_RUNTIME_DIR:-$RUNTIME_BASE/$PROJECT_NAME}"
ISO_PATH="${AB_ISO:-$SOURCE_BASE/$PROJECT_NAME/Atomic Bomberman.ISO}"
CDROM_DIR="${AB_CDROM_DIR:-$RUNTIME_DIR/cdrom}"
PREFIX_DIR="${AB_WINEPREFIX:-$RUNTIME_DIR/wineprefix32}"
ICON_SOURCE="${AB_ICON_SOURCE:-$CDROM_DIR/BM95.ICO}"

APPDIR_ONLY=0

source "$REPO_ROOT/scripts/wine-appimage-builder.sh"
wine_appimage_init_defaults

usage() {
  cat <<'EOF'
Brug:
  ./extras/build_appimage.sh [--appdir-only] [--no-download]

Bygger en selvstændig AppDir/AppImage for Atomic Bomberman.
Kør normalt først:

  ./install.sh --download --no-launch
  AB_MODE=prepare ./launch.sh

Scriptet pakker:
  - extracted CD-ROM runtime
  - seeded Wine prefix
  - host Wine runtime and ELF dependencies
  - AppImage launcher that stores mutable Wine prefix state in ~/.local/share

Miljøvariable:
  AB_ISO=...              ISO der kan udtrækkes hvis runtime mangler
  AB_RUNTIME_DIR=...      runtime mappe med cdrom/ og wineprefix32/
  AB_WINEPREFIX=...       prepared Wine prefix
  APPDIR=...              hvor AppDir bygges
  DIST_DIR=...            output mappe
  OUTPUT_APPIMAGE=...     endelig AppImage-sti
  APPIMAGETOOL_BIN=...    brug specifik appimagetool binær
  DOWNLOAD_APPIMAGETOOL=0 undgå auto-download

Bemærk:
  AppImage-filen kommer til at indeholde spilfiler og et prepared Wine prefix.
  Byg/distribuér den kun hvis du har rettigheder til den konkrete kopi.
EOF
}

while [[ $# -gt 0 ]]; do
  case "$1" in
    --appdir-only) APPDIR_ONLY=1 ;;
    --no-download) DOWNLOAD_APPIMAGETOOL=0 ;;
    -h|--help) usage; exit 0 ;;
    *) wine_appimage_fatal "Ukendt argument: $1" ;;
  esac
  shift
done

prepare_atomic_runtime() {
  if [[ -f "$CDROM_DIR/BM95.EXE" && -f "$CDROM_DIR/AUTORUN.INF" && -f "$PREFIX_DIR/system.reg" ]]; then
    return 0
  fi
  [[ -f "$ISO_PATH" ]] || wine_appimage_fatal "Runtime mangler og ISO blev ikke fundet: $ISO_PATH. Kør ./install.sh --download --no-launch først."
  wine_appimage_log "Forbereder Atomic Bomberman runtime via launch.sh prepare"
  AB_ISO="$ISO_PATH" AB_RUNTIME_DIR="$RUNTIME_DIR" AB_WINEPREFIX="$PREFIX_DIR" \
    AB_MODE=prepare "$GAME_DIR/launch.sh"
}

copy_atomic_game_files() {
  wine_appimage_log "Kopierer Atomic Bomberman runtime"
  install -Dm755 "$GAME_DIR/launch.sh" "$APPDIR/game/launch.sh"
  install -Dm644 "$GAME_DIR/README.md" "$APPDIR/game/README.md"
  install -Dm644 "$GAME_DIR/recipe.yml" "$APPDIR/game/recipe.yml"
  if [[ -f "$ISO_PATH" ]]; then
    install -Dm644 "$ISO_PATH" "$APPDIR/game/Atomic Bomberman.ISO"
  fi
  wine_appimage_sync_tree "$CDROM_DIR" "$APPDIR/game/cdrom"
  wine_appimage_sync_tree "$PREFIX_DIR" "$APPDIR/game/wineprefix"
  find "$APPDIR/game" -name '*.log' -delete || true
}

write_atomic_internal_launcher() {
  wine_appimage_log "Skriver Atomic Bomberman AppImage launcher"
  cat > "$APPDIR/game/appimage-launch.sh" <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail
HERE="${APPDIR:?APPDIR not set}"
APP_STATE_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/atomic-bomberman-1997"
mkdir -p "$APP_STATE_DIR"
# Keep the CD-ROM tree on the AppImage mount. launch.sh only writes
# .windows-label opportunistically and tolerates failure, so avoid copying the
# entire CD tree to writable state on every first run.
export AB_RUNTIME_DIR="$APP_STATE_DIR/runtime"
export AB_WINEPREFIX="$WINEPREFIX"
export AB_CDROM_DIR="$HERE/game/cdrom"
export AB_ISO="$HERE/game/Atomic Bomberman.ISO"
export AB_WINE_BIN="${WINE_BIN:?WINE_BIN not set}"
export AB_MODE="${AB_MODE:-game}"
export AB_VIRTUAL_DESKTOP="${AB_VIRTUAL_DESKTOP:-1}"
export AB_DESKTOP_SIZE="${AB_DESKTOP_SIZE:-800x600}"
export AB_WINEDEBUG="${AB_WINEDEBUG:--all}"
cd "$HERE/game"
exec "$HERE/game/launch.sh"
EOF
  chmod +x "$APPDIR/game/appimage-launch.sh"
}

validate_atomic_inputs() {
  wine_appimage_validate_base_tools
  prepare_atomic_runtime
  [[ -f "$CDROM_DIR/BM95.EXE" ]] || wine_appimage_fatal "Mangler BM95.EXE i $CDROM_DIR"
  [[ -f "$CDROM_DIR/AUTORUN.INF" ]] || wine_appimage_fatal "Mangler AUTORUN.INF i $CDROM_DIR"
  [[ -f "$CDROM_DIR/CFG.INI" ]] || wine_appimage_fatal "Mangler CFG.INI i $CDROM_DIR"
  [[ -f "$PREFIX_DIR/system.reg" ]] || wine_appimage_fatal "Mangler prepared Wine prefix i $PREFIX_DIR"
}

main() {
  validate_atomic_inputs
  wine_appimage_reset_dirs
  copy_atomic_game_files
  wine_appimage_copy_wine_runtime
  wine_appimage_collect_runtime_deps
  write_atomic_internal_launcher
  wine_appimage_write_runner_scripts
  wine_appimage_write_desktop_file
  wine_appimage_write_icon "$ICON_SOURCE"
  wine_appimage_verify_appdir
  if [[ "$APPDIR_ONLY" == "0" ]]; then
    wine_appimage_build_appimage
  fi
  wine_appimage_summarize
}

main "$@"