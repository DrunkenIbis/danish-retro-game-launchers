...
PROJECT_NAME=magnus-myggen-midnatsmysteriet
DISPLAY_NAME='Magnus & Myggen: Midnatsmysteriet'
RUNTIME="$(realpath -e -- "${MIDNIGHT_APPIMAGE_RUNTIME:-$REPO/local/runtime/magnus-myggen-midnatsmysteriet-download}")"
PREFIX="$RUNTIME/prefix-ge-xp"
GE="$REPO/local/cache/mm3-physical-cd/runner/lutris-GE-Proton7-43-x86_64"
INSTALLED='drive_c/Program Files/Magnus & Myggen - Midnatsmysteriet'
...
[[ ! -L $PREFIX && -s $PREFIX/system.reg && -s "$PREFIX/$INSTALLED/mm13main.exe" ]] || wine_appimage_fatal 'Original local-copy installation missing.'
...
# Hold the same lock as launch.sh through copying/building. Never kill it.
